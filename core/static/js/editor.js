...
document.addEventListener("DOMContentLoaded", () => {
	...
	let showingSubmissionNotification = false;
	const submissionNotification = document.getElementById("submission-notification");
	...
	function setupLanguage(language, force = false) {
		if (language !== "null") {
			...
			console.log(
				`Changed Language to:${language}:${editor.getOption("mode")} Identation: ${editor.getOption("indentUnit")}`,
			);
		}
	...
	function setupRun() {
		console.info("Setting up Run...");
		const runButton = document.getElementById("run-button");
		if (runButton) {
			runButton.addEventListener("click", function (event) {
				const lastSubmissionTimestamp = new Date(document.querySelector("[name='last-submission-timestamp']").value);
				const secondsSinceLastSubmission = (new Date() - lastSubmissionTimestamp) / 1000;

				if (secondsSinceLastSubmission < 15) {
					if (!showingSubmissionNotification) {
						let secondsPassed = Math.floor(secondsSinceLastSubmission);
						submissionNotification.innerHTML = `Espere ${15 - secondsPassed} segundos entre os envios.`;
						showingSubmissionNotification = true;
						submissionNotification.classList.remove("is-hidden");
						const interval = setInterval(() => {
							submissionNotification.innerHTML = `Espere ${15 - secondsPassed} segundos entre os envios.`;
							secondsPassed += 1;
						}, 1000);
						setTimeout(() => {
							showingSubmissionNotification = false;
							submissionNotification.classList.add("is-hidden");
							clearInterval(interval);
						}, 16000 - secondsPassed * 1000);
					}
	...
	if (saveButton && filenameField) {
		saveButton.addEventListener("click", (event) => {
			...
			fetch("/editor/save/", options).then((response) => {
