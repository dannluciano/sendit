...
@login_required
def editor(request):
    return render(request, "editor/runner.html")
...
def file_code_save(request):
    try:
        ...
        file_code, created = FileCode.objects.update_or_create(
            name=name, owner=owner, defaults=file_code_dict
        )

        return JsonResponse(
            {"status": "ok", "uuid": file_code.uuid, "created": created}
        )
    except Exception as e:
        return JsonResponse({"error": e.__class__.__name__, "msg": str(e)})
