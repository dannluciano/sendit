...
class Perfil(models.Model):
    ...
    def erros_de_sintax(self):
        return Submission.objects.filter(
            autor_id=self.user_id, status=Submission.STATUS_CHOICES[1][0]
        ).count()

    @property
    def erros_de_execucao(self):
        return Submission.objects.filter(
            autor_id=self.user_id, status=Submission.STATUS_CHOICES[2][0]
        ).count()

    @property
    def erros_de_tempo(self):
        return Submission.objects.filter(
            autor_id=self.user_id, status=Submission.STATUS_CHOICES[3][0]
        ).count()

    @property
    def erros_de_saida(self):
        return Submission.objects.filter(
            autor_id=self.user_id, status=Submission.STATUS_CHOICES[4][0]
        ).count()

    @property
    def acertos(self):
        return Submission.objects.filter(
            autor_id=self.user_id, status=Submission.STATUS_CHOICES[5][0]
        ).count()
